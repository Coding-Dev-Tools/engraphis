...
function managedConsentHtml(feature){return `<div class="empty" data-csp-style="s87"><div>${esc(feature)} needs your explicit permission before this installation uploads a bounded workspace snapshot.</div><div class="field-hint">Set <code>ENGRAPHIS_MANAGED_COMPUTE_CONSENT=1</code> in <code>.env</code>, then restart Engraphis. Secret and session-scoped memories remain excluded.</div></div>`}
function managedConsentRequired(error){return error&&CURRENT_VIEW!=='analytics'&&CURRENT_VIEW!=='automation'&&error.status===409&&error.detail&&error.detail.code==='consent_required'}
/* Entitlement failure — and only that. 401 unauthenticated, 402 subscription not active,
   501 feature not offered here: each means the customer's plan does not include what they
   just opened, so each may draw the purchase panel.
   409 is deliberately NOT here. Every 409 this API emits is a *conflict*, never a billing
   answer: detail.code==='consent_required' means the customer IS entitled but has not set
   ENGRAPHIS_MANAGED_COMPUTE_CONSENT=1 (managedConsentRequired, above), and the rest are
...
function hostedFeatureUnavailable(error){return !!error&&((CURRENT_VIEW==='analytics'||CURRENT_VIEW==='automation')||error.status===401||error.status===402||error.status===501)}
