...
function lockReason(team){const st=licAccessState(),ends=licTrialEnds();
 ...
 if(licTrialAvailable()){const days=licTrialDays(team?'team':'pro');return days?`The email-confirmed, no-card trial lasts exactly ${days} active days.`:'An email-confirmed, no-card trial is available; review its duration in Cloud.'}
...
function licTrialDays(plan){const trial=(LIC&&LIC.trial)||{},days=(trial.days_by_plan||{})[plan]??(plan==='pro'?trial.trial_days:null);return Number.isSafeInteger(days)&&days>0?days:null}
