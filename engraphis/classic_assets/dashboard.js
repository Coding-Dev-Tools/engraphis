...
function loadGraphEngine(){
 ...
 GRAPH_ENGINE_LOADING=new Promise((resolve,reject)=>{
  const script=document.createElement('script');
  script.src='/v2-assets/engraphis-graph.js?v=20260810-galaxy-explicit-reheat';
...
;(async()=>{try{const auth=await api('/auth/state'),m=document.getElementById('deployment-mode-indicator');if(m){const isLocal=auth.deployment_mode==='local';m.textContent=isLocal?'LOCAL':'HOSTED';m.title=isLocal?'Local mode: no hosted cloud configured':'Hosted mode: connected to Engraphis Cloud';m.className='deployment-mode '+(isLocal?'mode-local':'mode-hosted');m.hidden=false}}catch(e){}})();
