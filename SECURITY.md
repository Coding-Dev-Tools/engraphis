...
- **Grounded recall (`core/grounded.py`, `engraphis_recall_grounded`, reviewed 2026-07-05).**
  Read-only (`readOnlyHint`); no mutation, no hard delete. Adds no new untrusted-input surface:
  the service entry point (`MemoryService.grounded_recall`) reuses `recall`'s validation and the
  server-side workspace binding, so scope isolation is unchanged. The **default path invokes no
  LLM** — the answer is an extractive stitch of retrieved memories that never introduces an uncited
  claim, and the MCP tool never passes an `llm`. The **abstain gate** is itself a safety property:
  a poisoned-but-irrelevant memory can't force an answer just by being the nearest vector (it falls
  below `GROUNDED_SUPPORT_FLOOR`). The **optional** injected LLM synthesiser treats retrieved
  content as untrusted — the system prompt fences sources as data and instructs the model to ignore
  instructions inside them (indirect-prompt-injection defence, defence-in-depth not a guarantee),
  output is constrained to `[n]` citations and the abstain sentinel, and any LLM error degrades to
  the extractive answer. Residual, unchanged from the base threat model: a poisoned memory that *is*
  relevant to the query will be cited like any true memory — provenance on every citation keeps that
  auditable (§1).
