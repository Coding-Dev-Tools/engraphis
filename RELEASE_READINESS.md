...
our internal go-to-market notes, which are now explicit that the $20/mo Pro tier is a well-anchored
...
| Check | Status |
|------|--------|
| `pytest tests/` | **305 passed** with the `[test]` extras CI installs (fastapi/mcp/tree-sitter, no torch). On the numpy-only `core-py39` CI job the extras-gated tests (`test_app_auth.py`/`test_mcp_server.py`, …) skip via `importorskip`; everything else still runs and passes on `numpy` alone. |
...
**Competitive-feature pass (previous)** — closes the gap to mem0/Zep/Letta; full detail in
`CHANGELOG.md`:
...
- Clean rebrand to a first-party identity; `engraphis.db` default.
...
- GTM assets: internal go-to-market notes, `docs/landing-page.md`.
...
3. **Encryption-at-rest** — still open, rely on disk/FS encryption. **Built-in rate limiting**
   — *(RESOLVED — an optional in-process per-IP limiter now ships, `ENGRAPHIS_RATE_LIMIT`/
   `ENGRAPHIS_RATE_WINDOW`, off by default; still front multi-process/distributed deployments
   with a reverse proxy. See SECURITY.md §"Known limitations".)* Encryption-at-rest remains
   required for the regulated/Enterprise ICP.
...
5. **An actual Pro-tier feature to sell.** *(RESOLVED 2026-07-03 — see
   `CHANGELOG.md`: offline signed license keys now gate three
   ...
   $20/mo" — it's still a single-user local dashboard with no hosting, login, or team sync.
   A hosted + multi-user version of this dashboard is the shortest path to a first real Pro deliverable.
6. **CI matrix + wheel + Docker smoke** — *(DONE — `.github/workflows/ci.yml` now runs a 3.10/3.11/3.12 matrix plus a numpy-only 3.9 core-floor job, `ruff`, the extras-gated suite, a Docker build+health smoke test, and a wheel build+install.)* The one remaining step is publishing the wheel to PyPI — see `docs/RELEASE.md`.
7. **v1 input hardening** — *(RESOLVED — v1's request models now strip control characters and
   cap length on stored/name text fields, at parity with v2's `service.py`; the file-upload
   path caps body size too. See SECURITY.md §"Known limitations".)*
