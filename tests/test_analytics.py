...
# ── HTML report (Pro export) — same pure-function-first discipline ────────────────────

def _sample_analytics():
    rows = [_row(stability=5.0), _row(stability=1.0, last_access=NOW - 2.5 * 86400),
            _row(pinned=1), _row(valid_to=NOW - 10, mtype="episodic")]
    return analytics_from_rows(rows, {"invalidate": 2, "noop": 3},
                               [{"name": "Alice", "etype": "person", "n": 4}], now=NOW)


def test_render_analytics_html_covers_every_section_of_the_payload():
    page = render_analytics_html(_sample_analytics(), workspace="acme", version="9.9.9")
    assert page.startswith("<!doctype html>")
    for heading in ("Totals &amp; decay forecast", "Memories written per week",
                    "Retention distribution", "Live memories by type",
                    "Write-path resolver activity", "Most connected entities"):
        assert heading in page
    assert "acme" in page and "9.9.9" in page
    assert "Alice" in page and "invalidate" in page
    assert "2025-06-15" in page                      # generated_at = NOW, rendered UTC


def test_render_analytics_html_is_self_contained_and_escapes_content():
    data = _sample_analytics()
    data["top_entities"] = [{"name": "<script>alert(1)</script>", "etype": "p&q", "n": 1}]
    page = render_analytics_html(data, workspace="<b>ws</b>", version="1.0")
    assert "<script>" not in page and "&lt;script&gt;" in page
    assert "<b>ws</b>" not in page and "&lt;b&gt;ws&lt;/b&gt;" in page
    # nothing loads from the network: inline style only, no links, no images
    assert "<style>" in page
    for needle in ("<script", "src=", "href=", "@import", "url("):
        assert needle not in page


def test_render_analytics_html_handles_an_empty_workspace():
    data = analytics_from_rows([], {}, [], now=NOW)
    page = render_analytics_html(data, workspace="empty")
