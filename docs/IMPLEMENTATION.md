...
This is the runnable foundation for the v2 build. Phase 0 delivers the **architectural backbone**; Phase 1 delivers the **real
...
> what shipped when for shipped status; `AGENTS.md` §6 is the
> up-to-date phase tracker. The rest of this document is a historical snapshot of the Phase 0–1
> foundation and is still accurate for that layer. Remaining Phases 2–6 work follows the
> original v2 design roadmap (see `AGENTS.md` §6 for current status).
...
scripts/
└── migrate_to_v2.py    # v1 (engraphis_v1.db) → v2 scoped/bi-temporal migration
eval/
...
- **Hybrid arms (stubs ready)** — vector (`NumpyVectorIndex`) + lexical (`Store.fts_search`)
  + graph (`Store.neighbors`); Phase 1 fuses + reranks them (see `core/recall.py`).
...
│                #   → rerank → context packing → reinforcement
...
1. LLM fact extraction + ADD/UPDATE/NOOP/INVALIDATE conflict resolution (see `core/resolve.py`).
