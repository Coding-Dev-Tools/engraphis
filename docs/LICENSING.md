...
customer-side hosted-service protocols, and other code currently present here. Subject to the
...
The public package accepts only `ENGRAPHIS_SERVICE_MODE=customer`. It contains the local memory engine and customer clients; hosted service implementations are maintained separately and cannot be enabled through public-package configuration.

This deployment boundary does not change the Apache license or prevent a fork from modifying code already released here.
