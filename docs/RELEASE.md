...
- [x] **Fix the project URLs.** *(Done 2026-07-07 — `pyproject.toml [project.urls]`,
      `licensing.DEFAULT_UPGRADE_URL`, and `scripts/init.py` now all point at the real remote
      `github.com/Coding-Dev-Tools/engraphis`, so the in-product upgrade link and the PyPI page
      resolve.)* If you later create an `engraphis` GitHub org and move the repo there (nicer
      branding), flip those three references back to `github.com/engraphis/engraphis`.
...
- [ ] Tweet/post the one command. "`pip install engraphis`" *is* the launch.
