...
Closes the biggest first-run gap (docs/LAUNCH_PLAN.md §4.2): without configuration the
...
def cmd_check() -> int:
    ...
    try:
        ...
        if err:
            ...
            _ok("license", f"{lic.plan}" + (
                f" ({', '.join(sorted(lic.features))})" if lic.features else
                " — free tier, core fully functional"))
...
def _env_content(db_path: Path, token: str) -> str:
    ...
    lines += [
        "# Unlock Pro/Team (analytics, export, multi-user) — https://engraphis.dev/pro",
...
def main(argv=None) -> int:
    ...
    print("  Free forever at the core — Pro/Team unlock: https://engraphis.dev/pro")
    return 0
