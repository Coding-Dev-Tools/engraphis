...
$running = $false
try {
    $response = Invoke-WebRequest -Uri "$Url/memory/health" -UseBasicParsing -TimeoutSec 3 -ErrorAction Stop
...
if (-not $running) {
    ...
    Start-Process -FilePath $pythonExe `
        -ArgumentList "-m", "scripts.start_server" `
    ...
    for ($i = 0; $i -lt 30; $i++) {
        ...
        try {
            $response = Invoke-WebRequest -Uri "$Url/memory/health" -UseBasicParsing -TimeoutSec 2 -ErrorAction Stop
